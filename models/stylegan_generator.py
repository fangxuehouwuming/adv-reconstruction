...
class StyleGANGenerator(BaseGenerator):
  ...
  """
    ...
    def build(self):
        ...
        self.net = StyleGANGeneratorNet(resolution=self.resolution,
    ...
    """
        ...
        # Generate from W+ space.
            ...
            if (latent_codes.ndim != 3
                    or latent_codes.shape[1:] != (self.num_layers, self.w_space_dim)):
                raise ValueError(f'Latent codes should be with shape [batch_size, '
        ...
        if generate_style:
            for i in range(self.num_layers):
                style = self.net.synthesis.__getattr__(f'layer{i}').epilogue.style_mod.dense(
                    wps[:, i, :])
    ...
    def synthesize(self,
        ...
        return self.batch_run(
            latent_codes, lambda x: self._synthesize(x,
